...
export const takeSnapshot = async (imageData: string): Promise<Student[]> => {
  const response = await axios.post(`${API_BASE_URL}/api/attendance/snapshot`, { image: imageData }, { withCredentials: true });
...
export const fetchAttendanceRecords = async (): Promise<AttendanceRecord[]> => {
  const response = await axios.get<AttendanceRecord[]>(`${API_BASE_URL}/api/attendance/records`, { withCredentials: true });
  return response.data;
};

export const exportAttendanceToExcel = async (): Promise<void> => {
  const response = await axios.get(`${API_BASE_URL}/api/attendance/export`, { responseType: 'blob', withCredentials: true });
...
export const fetchAttendanceData = async (params: Record<string, any> = {}): Promise<Student[]> => {
  ...
  const response = await axios.get<Student[]>(`${API_BASE_URL}/attendance${query}`, { withCredentials: true });
  return response.data;
};

export const fetchAttendanceReports = async (params: Record<string, any>): Promise<AttendanceRecord[]> => {
  const response = await axios.get<AttendanceRecord[]>(`${API_BASE_URL}/api/attendance/reports`, { params, withCredentials: true });
  return response.data;
};

export const fetchStudents = async (params: Record<string, any> = {}): Promise<Student[]> => {
  const response = await axios.get<Student[]>(`${API_BASE_URL}/students`, { params, withCredentials: true });
  return response.data;
};
...
// Remove Axios interceptor for Bearer token handling
